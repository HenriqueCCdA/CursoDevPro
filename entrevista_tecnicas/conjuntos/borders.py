...
def has_move(board, line, column):
    '''Given a board with Black, White and Empty slots. This function must 
    ...
    so until there is no more Empty slot adjacent to any occupied one.
    ...
    if color == EMPTY:
        raise Exception('Initial position canno be empty')
    ...
    def has_empty_adjacent(position):
        return any(board[p_line][p_column] == EMPTY 
...
print(has_move(board_white_finished, 0, 3))