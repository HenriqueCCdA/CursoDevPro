def potencia_recursiva_linear(base: int, expoente: int):
    '''
    f(n) = 4 + f(n-1)
    f(n) = 4 + 4 + f(n-2)
    f(n) = 4 + 4 + + 4 + f(n-4)
    f(n) = 4 * n

    O(n) = n em tempo de execução
    O(n) = n para memoria (sem otimização de recursão de cauda)
    '''
    if expoente == 0:
        return 1

    return base * potencia_recursiva_linear(base, expoente - 1)


def _potencia_recursiva_linear(base, expoente, resultado=1):
    if expoente == 0:
        return resultado
    return _potencia_recursiva_linear(base, expoente-1, resultado * base)


def potencia_recursiva_linear_v2(base: int, expoente: int):
    return _potencia_recursiva_linear(base, expoente - 1, base)


def potencia_iterativa(base: int, expoente: int):
    '''
    f(n) = 4 + f(n-1)
    f(n) = 4 + 4 + f(n-2)
    f(n) = 4 + 4 + + 4 + f(n-4)
    f(n) = 4 * n

    O(n) = n em tempo de execução
    O(1) = 1 para memoria
    '''
    resultado = 1
    for _ in range(expoente):
        resultado *= base

    return resultado


def _potencia_recursiva_logaritmica(base, expoente, resultado=1):
    '''
    base ** expoente = se expoente é par : base ** (2 * n), onde n é expoente // 2
    Desenvolvendo:
    (base ** 2) ** n =  (base * base) ** n

    caso contrário expoente = 2*n + 1 base ** (2*n + 1), onde n é expoente // 2
    Desenvolvendo
    base ** (2*n + 1) -> base * (base ** (2 * n)) = base * [(base * base)**n]

    O(log n) para tempo de execução
    O(log n) para memoria

    '''
    if expoente == 0:
        return resultado

    if expoente % 2 == 0:
        return _potencia_recursiva_logaritmica(base * base, expoente//2, resultado)
    else:
        resultado *= base
        return _potencia_recursiva_logaritmica(base, expoente-1, resultado)



def potencia_recursiva_logaritmica(base: int, expoente: int):
    return _potencia_recursiva_logaritmica(base, expoente)


if __name__ == '__main__':
    print(potencia_iterativa(2, 10))
    print(potencia_recursiva_linear(2, 10))
    print(potencia_recursiva_linear_v2(2, 10))
    print(potencia_recursiva_logaritmica(2, 2))
    print(potencia_recursiva_logaritmica(2, 3))
    print(potencia_recursiva_logaritmica(2, 8))
    print(potencia_recursiva_logaritmica(2, 10))